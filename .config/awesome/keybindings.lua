...
M.global_keys = gears.table.join(
    awful.key({ modkey }, 'p', function() awful.spawn(config.dmenucmd) end),
    awful.key({ modkey }, 'o', function() awful.spawn(config.termcmd) end),
    awful.key({ modkey, 'Shift' }, 'o', function() awful.spawn(config.termtmuxcmd) end),
...
for i, v in ipairs({ '=', '-', 0, 9, 8, 7, 6, 5, 4 }) do
    M.global_keys = gears.table.join(M.global_keys,
        awful.key({ modkey }, v, function() M.view_only(i) end),
        awful.key({ modkey, 'Control' }, v, function() M.view_toggle(i) end),
        awful.key({ modkey, 'Shift' }, v, function() M.move_to_tag(i) end),
        awful.key({ modkey, 'Control', 'Shift' }, v, function() M.toggle_tag(i) end)
    )
end
